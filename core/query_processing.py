...
# # In query_processing.py
# FILE_TOPICS_PATH = Path(DATA_DIR) / "file_topics.json"

# def save_file_topics(file_id: str, topics: Set[str]):
#     """Save topics associated with a specific file."""
#     os.makedirs(FILE_TOPICS_PATH.parent, exist_ok=True)
#     file_topics = {}
    
#     if FILE_TOPICS_PATH.exists():
#         try:
#             with open(FILE_TOPICS_PATH, 'r') as f:
#                 file_topics = json.load(f)
#         except Exception as e:
#             print(f"Error loading file topics: {e}")
    
#     # Save this file's topics
#     file_topics[file_id] = list(topics)
    
#     # Update global topics list
#     all_topics = set()
#     for topic_list in file_topics.values():
#         all_topics.update(topic_list)
    
#     # Save updated file topics
#     with open(FILE_TOPICS_PATH, 'w') as f:
#         json.dump(file_topics, f)
    
#     # Update document topics for query processing
#     update_document_topics(all_topics)

# def remove_file_topics(file_id: str):
#     """Remove topics associated with a specific file from the global index."""
#     if not FILE_TOPICS_PATH.exists():
#         return False
...
#     # Remove this file's topics
#     if file_id not in file_topics:
#         return False
    
#     file_topics.pop(file_id, None)
...
class QueryIntentClassifier:
    ...
    def load_data(self):
        """Load saved thresholds, feedback, and file topics."""
        # Load threshold and feedback data
        if FEEDBACK_PATH.exists():
            try:
                with open(FEEDBACK_PATH, 'r') as f:
    ...
    def get_all_topics(self) -> Set[str]:
        """Get a set of all topics from all files."""
        all_topics = set()
        for topic_list in self.file_topics.values():
            all_topics.update(topic_list)
        return all_topics
    ...
    def needs_document_search(self, query: str) -> bool:
        ...
        # Stage 2: Topic matching
        query_topics = self.extract_topics(query)
        all_document_topics = self.get_all_topics()
        
        # If query contains topics that match our document topics, likely needs search
        if query_topics and all_document_topics:
            for topic in query_topics:
                if topic in all_document_topics or any(topic in doc_topic for doc_topic in all_document_topics):
                    return True
...
def save_file_topics(file_id: str, topics: Set[str]):
    ...
    # Update classifier's file topics
    query_classifier.file_topics = file_topics

def remove_file_topics(file_id: str):
    """Remove topics associated with a specific file."""
    if not FILE_TOPICS_PATH.exists():
        return False
